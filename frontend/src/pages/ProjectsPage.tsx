...
const TABLE_COLUMNS: ColumnDef<Project>[] = [
  {
    ...
    render: (_, row) => (
      <div>
        <div className="text-[13px] font-semibold text-slate-800 line-clamp-1">{row.name}</div>
        {row.description && (
          <div className="text-[11px] text-slate-400 line-clamp-1 mt-0.5">{row.description}</div>
        )}
      </div>
    ),
  },
  ...
  },
  {
    ...
    align: 'center',
    sortable: true,
    render: (v) => <span className="text-sm font-semibold text-slate-700">{formatNumber(Number(v ?? 0))}</span>,
  },
  {
    key: 'session_count',
    header: 'Sessões',
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="relative flex-1 min-w-[200px] max-w-sm">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 text-sm bg-white rounded-lg border border-slate-200 text-slate-700 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400 transition-all"
            />
          </div>
          ...
          <div className="relative">
            ...
            {statusOpen && (
              <>
                ...
                <div className="absolute left-0 top-full mt-1 z-20 w-44 py-1 bg-white rounded-xl border border-slate-200 shadow-dropdown animate-scale-in">
          ...
          <div className="card overflow-hidden">
            <DataTable
              ...
              rowActions={(row) => [
                ...
                { label: 'Arquivar', onClick: () => {} },
                { label: 'Excluir', onClick: () => {}, destructive: true },
              ]}
              emptyState={
